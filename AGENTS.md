...
- Consult [odin_port/VALIDATION.md](odin_port/VALIDATION.md) for the established baseline and instrumentation evidence. Update it after a validated fix, and update issue status when an active tracker exists; distinguish new regressions from retained baseline diagnostics. Reuse working instrumentation without repeating its setup probes unless the environment or collection path changes.
