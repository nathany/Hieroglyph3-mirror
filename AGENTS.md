...
- Record accepted deviations in reader-facing sample notes and concise matching code comments. Explain the reference behavior and reason for the departure near the affected code; keep inherited limitations distinct from improvements. Keep resolved issue IDs and detailed test history in Git history, not in reader-facing explanations. Prefer a clear error and clean exit for an unrecoverable demo failure; a general retry or device-recovery framework is not required.
...
- Use the tracking allocator and Direct3D debug layer when the affected path needs leak, lifetime, or API validation. Track shader compiler blobs separately: D3D live-device-object reports do not cover their ownership, and the Odin allocator does not cover native COM allocations. Compiler checks and sanitizer builds complement review; they do not establish behavioral equivalence by themselves.
...
- Record the commands, runtime paths, results, and material limits of each change in the task report or commit description; update issue status when an active tracker exists. Reuse working instrumentation without repeating its setup probes unless the environment or collection path changes. Distinguish new regressions from the retained diagnostics below.
- Prefer one semantic fix per commit after relevant validation. Keep shared API-contract changes and their callers together; use small batches only when they share a cause or narrowly shared test scope. Retest all 14 rendering demos for shared renderer startup changes; target local fixes to their affected demos and modes. BasicWindow does not use Direct3D. Comment-only and documentation changes do not require a fresh graphical baseline; verify affected examples, local links, and factual claims instead.

### Targeted runtime coverage

- Shared resize changes affect ImmediateRenderer, ImageProcessor, ParticleStorm, WaterSimulation, DeferredRendering, LightPrepass, and SkinAndBones. Exercise resize/minimize/restore and dependent targets; inject failure when changing failure handling, since successful resizing does not cover it.
- For shader or binding changes, exercise affected stages and compiled modes: terrain has three shading variants and two hull modes; DeferredRendering has G-buffer, lighting, display, and AA combinations. Test WaterSimulation's FL10/SM4 capability path and ParticleStorm's `-define:DEBUG_COUNTS=true` when those paths change.
- For parser changes, test malformed/truncated input and valid bundled assets in affected consumers. For ownership changes, cover partial construction and successful siblings; for scratch-lifetime changes, repeat the allocating action and measure outstanding storage. Keep probes isolated, and state whether failures were injected or naturally observed. Run ASan standalone if debugger interception makes its result ambiguous.

### Established baseline and retained evidence

- The completed audit/fix baseline is `c6b394a`, tested on Windows with Odin `dev-2026-09-nightly:a2fb372`. Across that sequence, all 15 demos were run, all 14 rendering demos received shared-startup checks, and targeted controls, resize, ASan, allocator, and failure probes were exercised. `just verify` passed 15 strict application checks and eight math tests. This is historical evidence, not proof that later changes pass.
- Visual comparison used existing C++ binaries, not rebuilt references, at 150% DPI. Animations were not synchronized. FL10 testing used the installed AMD hardware; constrained-window and failure probes included synthetic conditions. Other drivers, exhaustive mode combinations, and long-duration stress were not established. The C++ binaries normally exit with code 1; Odin exits with 0.
- D3D instrumentation was confirmed through the actual device debug flag, debug interfaces, controlled diagnostics, native `OutputDebugString` collection, and live-object reporting. RenderDoc 1.46 was installed but capture/replay was not validated.
- Retained diagnostics: LightPrepass's stencil-only mask pass reports `DEVICE_DRAW_RENDERTARGETVIEW_NOT_SET`; Curved PN adaptive mode reports `DEVICE_DRAW_HULL_SHADER_INPUT_TOPOLOGY_MISMATCH` (six HS input points versus three IA patch points). The latter is an inherited rendering defect, not a harmless warning. ParticleStorm's earlier UAV hazard and optional readback leak were fixed; do not accept them as baseline exceptions. Other behavioral limitations remain in the [sample notes](odin_port/README.md).
- Recover the full historical validation log with `git show 1cb6836:odin_port/VALIDATION.md`, and the retired issue tracker with `git show c6b394a:KNOWN_ISSUES.md`. Local scripts, logs, binaries, and captures were stored under `%USERPROFILE%/.codex/visualizations/2026/09/06/01a078e7-6aba-7140-bd26-d0e1d547b055/`: `runtime-baseline-9a91a68/`, `instrumentation-ki001/`, `p2-fixes/`, and `p3-fixes/`. These artifacts are not distributed with the repository; check availability before relying on them.
